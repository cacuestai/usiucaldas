...
class Bloque {
    ...
    public function getSelect($param) {
        ...
        foreach ($conexion->getPDO()->query("SELECT nombre_bloque || ' - ' || nombre_sede AS data_bloque FROM bloque ORDER BY nombre_bloque ASC") as $fila) {
            $select .= "<option value='{$fila['data_bloque']}'>{$fila['data_bloque']}</option>";
        }
