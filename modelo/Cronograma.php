...
class Cronograma {
    ...
    function add($param) {
        ...
        if (($rs = $conexion->getPDO()->query($selectSQL))) {
            ...
            if ($mensaje) {
                $mensaje = __METHOD__ . " >> Fallo en la inserción de los siguientes registros:\n$mensaje";
                echo $mensaje;
                error_log($mensaje);
            }
        }
    ...
    function checkCollision($inicio, $fin, $rs) {
        while ($fila = $rs->fetch(PDO::FETCH_ASSOC)) {
            if ($this->check_range($fila['fecha_ini_prestamo'], $fila['fecha_fin_prestamo'], $inicio) || $this->check_out_range($fila['fecha_ini_prestamo'], $fila['fecha_fin_prestamo'], $fin) || $this->check_range($inicio, $fin, $fila['fecha_ini_prestamo']) || $this->check_out_range($inicio, $fin, $fila['fecha_fin_prestamo'])) {
                return TRUE;
            }
        }
        return FALSE;
    ...
    public function getProgramacion($param) {
        ...
        if (!isset($start) || !isset($end)) {
            error_log(__METHOD__ . "No se ha proporcionado un rango de datos");
        }
